      ...
      res.end = function (chunk: any, ...args: any[]): any {
        ...
        if (responseBody.length <= (config.responseBodySizeLimit || 0)) {
          try {
            ...
            const sanitizedBody = sanitizeObject(parsedBody, config.excludeResponseFields || []);
            ...
            logger.debug('Response body (raw)', {
              requestId,
              body: truncateString(responseBody, config.responseBodySizeLimit || 0)
            });
          }
        } else {
...
return oldEnd.apply(res, [firstArg, encoding, callback]);
    ...
    res.on('finish', () => {
      ...
      const responseSize = res.get('content-length') ? 
        parseInt(res.get('content-length')as string, 10) : 0;
      ...
      logger.info('Request completed', {
        ...
        duration,
        responseSize,
        responseHeaders: sanitizeObject(res.getHeaders(), config.excludeResponseFields || [])
      });
