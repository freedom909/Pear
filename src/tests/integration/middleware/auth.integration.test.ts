...
const createTestApp = () => {
  ...
  app.get('/api/auth/editor-or-admin', hasRole(['editor', 'admin']), (req: Request, res: Response) => {
    res.json({ message: '编辑者或管理员访问成功', user: req.user });
  });
  
  // 3. 资源所有者检查
  app.get('/api/resources/:resourceId', isResourceOwner(async (req: Request) => {
...
describe('Auth Middleware Integration Tests', () => {
  ...
  describe('isResourceOwner', () => {
    it('应该允许资源所有者访问自己的资源', async () => {
      ...
      expect(response.body).toEqual({
        message: '资源访问成功',
        resource: expect.objectContaining({ id: 'resource-1', ownerId: 'user-id' })
      });
    });
