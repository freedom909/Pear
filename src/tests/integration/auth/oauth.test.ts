...
describe('OAuth Authentication Flow', () => {
  ...
  describe('Google OAuth', () => {
    ...
    it('应该处理Google回调并重定向到首页', async () => {
      ...
      expect(passport.authenticate).toHaveBeenCalledWith('google', { failureRedirect: '/login' });
  ...
  describe('Facebook OAuth', () => {
    it('应该重定向到Facebook认证页面', async () => {
      ...
      expect(passport.authenticate).toHaveBeenCalledWith('facebook', { scope: ['email'] });
      expect(response.status).toBe(200);
    });
    ...
    it('应该处理Facebook回调并重定向到首页', async () => {
      ...
      expect(passport.authenticate).toHaveBeenCalledWith('facebook', { failureRedirect: '/login' });
      expect(response.status).toBe(302);
      expect(response.headers.location).toBe('/');
    });
  });
  ...
  describe('handleOAuthUser Service', () => {
    ...
    it('应该处理无效的配置文件数据', async () => {
      ...
      (handleOAuthUser as jest.Mock).mockRejectedValue(new Error('Invalid profile data'));
      
      await expect(handleOAuthUser(mockProfile as any, mockTokenInfo)).rejects.toThrow();
    });
  });
});