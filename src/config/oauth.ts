...
export class OAuthConfiguration {
  ...
  static getConfigs(): Record<string, OAuthConfig> {
    try {
      ...
      const applePrivateKeyLocation = EnvConfig.get('APPLE_PRIVATE_KEY_LOCATION');
      
      if (appleClientID && appleClientSecret && appleCallbackURL && 
          appleTeamID && appleKeyID && applePrivateKeyLocation) {
        configs.apple = {
          provider: 'apple',
      ...
      if (Object.keys(configs).length === 0) {
        logger.warn('No OAuth configurations loaded. Check your environment variables.');
      } else {
        logger.info(`Loaded OAuth configurations for providers: ${Object.keys(configs).join(', ')}`);
      }

      return configs;
    } catch (error) {
      logger.error('Failed to load OAuth configurations:', error);
      throw new Error('Failed to load OAuth configurations. Check your environment variables.');
    }
  }
}